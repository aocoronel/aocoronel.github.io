...
(define-abbrev-table 'odin-mode-abbrev-table
  '(("main" "main :: proc() {\n    @\n    return\n}" rc/abbrev-goto)))
...
(defun rc/zoxide-add-current-directory ()
  (when-let ((dir default-directory))
    (zoxide-add dir)))

(add-hook 'dired-mode-hook #'rc/zoxide-add-current-directory)
...
(use-package odin-mode
  :ensure nil
  :defer t
  :hook (odin-mode . odin-format-on-save-mode)
  :bind
...
(use-package mu4e
  ...
  :config
  (load-file "~/stow/mu4e/mu4e-config.el"))
